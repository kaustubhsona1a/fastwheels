...
export default function VehicleDetails() {
  ...
  if (!car) {
    if (loading) {
      return (
        <div className="min-h-screen bg-transparent flex flex-col items-center justify-center text-white font-sans relative">
    ...
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center p-20 text-zinc-650">
        <Helmet>
          <title>Vehicle Not Found | FAST WHEELS</title>
  ...
  const handleWhatsApp = () => {
    const message = encodeURIComponent(`Hi, I'm interested in the ${car.year} ${car.make} ${car.model} (${car.variant}) listed at ${formatPrice(car.price)} on FAST WHEELS website. Please share more details.`);
    window.open(`https://wa.me/918169423018?text=${message}`, '_blank');
  };

  const handleWhatsAppPhotos = () => {
    const message = encodeURIComponent(`Hi, I'm interested in the ${car.year} ${car.make} ${car.model} (${car.variant}) listed at ${formatPrice(car.price)} on FAST WHEELS website. Please share more photos of this vehicle.`);
    window.open(`https://wa.me/918169423018?text=${message}`, '_blank');
  };

  const handleCall = () => {
    window.open(`tel:+918169423018`);
  };
  ...
  const renderPriceBox = (isMobile: boolean) => {
    if (!car) return null;
    return (
      <div className={`bg-[#171717] border border-[#C1121F]/30 relative rounded-2xl shadow-xl backdrop-blur-md ${isMobile ? 'p-5' : 'p-10'}`}>
        <div className={`absolute top-0 right-0 bg-[#C1121F] text-white font-mono font-bold tracking-widest uppercase border-b border-l border-[#E63946]/40 ${isMobile ? 'text-[8px] px-3 py-1.5 rounded-bl-xl rounded-tr-2xl' : 'text-[10px] px-4 py-2 rounded-bl-xl rounded-tr-2xl'}`}>
          Verified Asset
        </div>
        ...
        <p className={`tracking-widest uppercase text-[#E63946] font-mono ${isMobile ? 'text-[9px] mt-1.5 mb-4' : 'text-[10px] mt-3 mb-8'}`}>{car.variant}</p>
        <div className={`font-bold text-[#E63946] border-b border-[#C1121F]/20 font-serif ${isMobile ? 'text-2xl pb-4 mb-4' : 'text-3xl pb-6 mb-4'}`}>{formatPrice(car.price)}</div>

        {car.instagramReel && (
          <div className={`rounded-2xl bg-gradient-to-r from-[#C1121F]/10 to-zinc-950 border border-[#C1121F]/30 flex flex-col ${isMobile ? 'mb-4 p-3 gap-2' : 'mb-8 p-4.5 gap-3.5'}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-zinc-400 uppercase tracking-wider font-mono ${isMobile ? 'text-[8px]' : 'text-[9px]'}`}>Watch immersive video review</p>
              </div>
            </div>
            <button
        ...
        <p className={`tracking-wider text-zinc-300 flex items-center font-mono font-semibold ${isMobile ? 'text-[10px] mb-4' : 'text-xs mb-8'}`}>
          <a href="https://maps.app.goo.gl/4VsYDmnUw519xPWz6?g_st=ac" target="_blank" rel="noreferrer" className="hover:text-[#E63946] transition-colors duration-300 inline-flex items-center">
            <MapPin className="w-4 h-4 mr-2 text-[#E63946]" /> Showroom, Byculla, Mumbai
          </a>
        </p>

        <div className={`space-y-2.5 font-mono uppercase font-bold ${isMobile ? 'text-[10px]' : 'text-[11px]'}`}>
          <button onClick={handleCall} className={`w-full bg-[#C1121F] hover:bg-[#FF3B3B] text-white rounded-xl transition-all duration-300 border border-[#E63946]/40 shadow-lg ${isMobile ? 'py-3' : 'py-4.5'}`}>
            Call Us Now
          </button>
          <button onClick={handleWhatsApp} className={`w-full bg-[#050505] hover:bg-[#121212] text-white border border-[#C1121F]/40 hover:border-[#E63946] rounded-xl transition-all duration-300 flex items-center justify-center gap-2 ${isMobile ? 'py-3' : 'py-4.5'}`}>
            <svg className="w-4 h-4 fill-current text-[#25D366]" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397 0 12.008 0c3.205.001 6.216 1.25 8.484 3.52 2.268 2.27 3.516 5.283 3.515 8.491-.005 6.655-5.344 12.003-11.95 12.003-.111 0-.221 0-.332-.005l-5.69 2.12c-.22.08-.454.04-.63-.12l-.35-.35zM6.57 17.51l.36.21c1.55.93 3.32 1.42 5.15 1.42a9.92 9.92 0 0 0 9.95-9.94c0-2.65-1.03-5.15-2.9-7.02C17.26 3.2 14.77 2.17 12.1 2.17 6.64 2.17 2.2 6.61 2.2 12.07c0 1.93.53 3.82 1.54 5.43l.23.37-1.01 3.69 3.61-.95zM17.43 14.93c-.29-.15-1.74-.86-2.01-.96-.27-.1-.47-.15-.67.15-.2.3-.77.96-.94 1.16-.17.2-.35.22-.64.07a8.1 8.1 0 0 1-2.39-1.48 8.94 8.94 0 0 1-1.65-2.05c-.17-.3-.02-.46.13-.61.13-.13.29-.34.44-.51.15-.17.2-.29.3-.49.1-.2.05-.37-.02-.52-.07-.15-.67-1.62-.92-2.22-.24-.59-.49-.51-.67-.52l-.57-.01c-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48 0 1.47 1.07 2.89 1.22 3.1 1.05 1.41 1.74 1.74 3.1 2.45a9.5 9.5 0 0 0 3.7.8c1.3-.01 2.44-.45 2.74-1 .3-.53.3-1 .22-1.12-.08-.12-.3-.19-.59-.34z"/></svg>
            Shoot WhatsApp Inquiry
          </button>
          <button onClick={handleShare} className={`w-full bg-[#050505]/60 hover:bg-[#121212] text-zinc-200 hover:text-white border border-[#C1121F]/30 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-sm ${isMobile ? 'py-3' : 'py-4.5'}`}>
  ...
  const renderEMICalculator = (isMobile: boolean) => {
    if (!car) return null;
    return (
      <div className={`bg-[#171717] border border-[#C1121F]/30 relative rounded-2xl shadow-xl backdrop-blur-md ${isMobile ? 'p-5 space-y-4' : 'p-8 space-y-6'}`}>
        <h2 className={`font-serif font-bold text-white uppercase tracking-widest border-b border-[#C1121F]/20 flex items-center justify-between ${isMobile ? 'pb-2 text-xs' : 'pb-3 text-sm'}`}>
          <span>EMI ESTIMATE CALCULATOR</span>
          <span className="text-[9px] uppercase tracking-widest font-mono text-[#E63946]">Live</span>
        </h2>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="text-[#E63946] font-bold font-sans">{formatPrice(loanAmount || Math.round(car.price * 0.8))}</span>
          </div>
          <input 
            type="range"
            ...
            className="w-full h-1 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-[#C1121F] focus:outline-none"
          />
        ...
        <div className={`bg-[#0A0A0A] border border-[#C1121F]/40 rounded-xl text-center shadow-inner relative overflow-hidden ${isMobile ? 'p-4 space-y-3' : 'p-5 space-y-4'}`}>
          <div className="space-y-0.5">
            ...
            <span className={`font-serif font-extrabold text-[#E63946] block ${isMobile ? 'text-2xl' : 'text-3xl'}`}>{formatPrice(monthlyEmi)}</span>
          </div>
          
          <div className="grid grid-cols-2 gap-3 pt-3 border-t border-[#C1121F]/20 text-[9px] sm:text-[10px] text-zinc-400 font-mono tracking-wider uppercase font-semibold">
  ...
  const pageTitle = `${car.year} ${car.make} ${car.model} ${car.variant} | FAST WHEELS`;
      ...
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="mt-6 md:mt-10 bg-zinc-900/55 p-5 md:p-10 border border-zinc-900/80 rounded-2xl relative overflow-hidden group hover:border-white/30 transition-colors duration-500 shadow-sm backdrop-blur-md">
          ...
          <p className="text-zinc-400 mb-2 max-w-xl text-xs md:text-sm font-light leading-relaxed">
            Every vehicle listed by the FAST WHEELS platform undertakes a specialized physical inspection covering diagnostic checks, accident/flood history, mileage validation, and comprehensive ownership verification.
          </p>
        </div>
          ...
          <div className="bg-zinc-900/90 border border-zinc-800/80 rounded-2xl p-6 sm:p-8 max-w-md w-full relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 backdrop-blur-xl text-zinc-300">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <button
                  onClick={() => {
                    const msg = `Check out this gorgeous ${car.year} ${car.make} ${car.model} listed at ${formatPrice(car.price)} on FAST WHEELS!`;
